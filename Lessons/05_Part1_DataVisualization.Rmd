...
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) it's inside the brackets so it's like adding another layer to the plot
...
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) + #max value for x axis added
  ylim(0, 20)
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() # to reflect depth
print(tempvsdepth)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? - It changes the fill of the boxplot
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) # not totally sure what is going on here
print(Nutrientplot4)
