...
Comment1: Understand the difference between the | and & symbol; and git hygiene 

Opening discussion: why might we be interested in long-term observations of temperature, oxygen, and light in lakes?

> Add notes here: They have messed with biota in Peter Lake but not in the other. Have a look at the metadata for more.
...
# dplyr filtering
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) # double equal sign

# Choose multiple conditions to filter
summary(NTL.phys.data$lakename) # you can also get a sense of how many fields by using unique and levels - as we used stringsAsFactors
NTL.phys.data.PeterPaul <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
# gives us peter or paul values
...
Exercise1 <- filter(NTL.phys.data,year4==1999)
...
Exercise5 <- pivot_longer(NTL.phys.data,9, names_to = "nutrient", values_to = "concentration")
