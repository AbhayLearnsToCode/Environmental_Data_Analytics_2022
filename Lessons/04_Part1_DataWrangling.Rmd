...
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE) #Ok this means summary works for character entries. 
library(lubridate)
...
class(NTL.phys.data$sampledate) # is factor the same as chr?
...
```{r, results = "hide"}
vignette("dplyr") # 'vignette' gives you an overview of the package
```
...
`!=`not equal
`<`less
`<=`is? no less than only
`>`greater
`>=`greater or equal
`&`if we want both to be true
`|`IF only one is to be true
...
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25) # because .25 is practically at the surface
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # the percent thing allows you to create a vector with what you want to include
