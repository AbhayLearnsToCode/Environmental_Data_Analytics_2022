...
Notes:
Residuals - the lower it is, the better our model is
Leverage - if a data point has major influence on our results
Transforming the data - going logarithmic with the data - review
Null hyp in ANOVA - no variance
Tukey - tells us about the distribution of populations 
...
> Answer: The AQI emissions are 50 ppm. H alt - they are less than 50 (one sided)
...
summary(EPAair$Ozone) # the sample is within the range of what we want - the t-test tells us if we can extrapolate this to the broader data
EPAair.subsample <- sample_n(EPAair, 5000) # shapiro can run only 5000 so we do this

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone)) #indicates that the results are NOT normal
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) # we WANT this to be a nice diagonal. In this case it is not.

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample # one sample compares it to one value - null is the diff between sample mean and the value is zero

# two sided - the diff is not zero
...
> One sided t-test indicated that Ozone levels are below the 50 ppm threshold (p-value <2.2e-16). Failed the Shapiro test

Reminder: p value is the likelihood that the null hypothesis is true
...
If we have more than two samples, we use an ANOVA
...
Ozone.regression <- lm(data = EPAair, Ozone ~ PM2.5)
...
Ex5 <- cor(EPAair %>% select(SITE_LATITUDE:PM2.5) %>% drop_na())
