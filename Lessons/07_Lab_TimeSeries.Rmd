...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #like text to columns in excel. First column null, second column has the month, the second one just says average, so we call it null2
  mutate( Date = my(paste0(Month,"-",Year))) %>% #paste0 concatenates month and year 
...
#column bind shouldn't used in TSA
...
Rainfall_TS <- ts(BRA_complete$Rainfall, start(fyear,fmonth),frequency=12)
Temp_TS <- ts(BRA_complete$Temperature_C, start(fyear,fmonth),frequency=12)
