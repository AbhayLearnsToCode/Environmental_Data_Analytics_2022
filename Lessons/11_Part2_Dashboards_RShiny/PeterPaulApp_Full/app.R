...
## Abhay:Tried Cerulean, Superhero, Slate and a few others.

# How do you change the default settings? 
## Changing the theme next to the "shinytheme" code

# How does each type of widget differ in its code and how it references the dataframe?
## Two widgets have x and y reference inputs which output the overall results,
## the other widgets are filters that help obtain nuanced insights.

#2. How is the mainPanel component of the UI structured? 
# How does the output appear based on this code?
## The mainPanel component leads with a plot output, and then a table output. 
## Bit unsure of what the table output is. The plot output syntax selects the 
## shape and form of the output.

#3. Explore the reactive formatting within the server.
# Which variables need to have reactive formatting? 
## The nutrients, dates, depthIDs, and lake names.

# How does this relate to selecting rows vs. columns from the original data frame?
# These variables ar eanalogous to column headings in a dataset
...
## y is based on the dropdown option the user selects for nutrients;the x input is
## the date range. The depth ID and lake name are filters. 
