...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", degree,",", R[2],'=0.6')))
print(Plot_text)
...
PeterPaulplot1 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = daynum, y = temperature_C, color = as.factor(year4))) +
  geom_point() +
...
PeterPaulplot2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, color = as.factor(depth))) +
  geom_point() +
  facet_wrap(vars(lakename), nrow = 2)
print(PeterPaulplot2)
