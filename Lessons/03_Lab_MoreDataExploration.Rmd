...
Numbers come in integers and 'doubles' (with decimals)

Characters - can come in strings

Boolean - True or false

Date is another type
...
vector1 # this gets the values into the console. Giving a print command will render better formatting 
vector2 <- c("one","two","three") # character vector
vector2
vector3 <- c(TRUE,TRUE,TRUE,FALSE,TRUE,FALSE) #logical vector
vector3

vector1[3] # give me the third item in this vector 
...
matrix1[4, ] # Gives you data from every column if you do not specify one
matrix1[ , 3] # Gives you data from every row 
matrix1[c(12, 14)] # twelfth AND fourteenth value
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
...
list1[[2]] # remember to refer to this for syntax!

# Data Frames ---- there is some functional overlap with matrices
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # the view command just opens up the popup window on R Studio
dataframe1[1:2,] # 
...
```{r}
vector4 <- c(45,50,33,43,51,54,59,66,57,42)
vector4
vector5 <- c(26,27,15,25,26,35,47,53,30,22)
vector4
...
CelsiusHigh <- FtoC(vector4)
CelsiusHigh

CelsiusLow <- FtoC(vector5)
CelsiusLow
```
...
g <- vector4 # What type of vector?
h <- vector5
i <- CelsiusHigh
j <- CelsiusLow
dataframe2 <- data.frame(g,h,i,j) #
names(dataframe2) <- c("F High","F Low","C High", "C Low"); View(dataframe2) # the view command just opens up the popup window on R Studio
